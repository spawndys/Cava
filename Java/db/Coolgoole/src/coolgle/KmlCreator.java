 ...
 ** Decription- File that modifies and a kml file to
 ** user preferences to interact with Google Earth
***********************************************/
...
public class KmlCreator 
{
	public static final void main (String[] argv) throws IOException, XPathExpressionException, ParserConfigurationException, SAXException
	{
		...
		String[] Locations = new String[]{"Anne Arundel Community College","UMBC","Annapolis Mall","Arundel High School"};

		// Turn the array of location names into a lat/lng array using the GetData Class:  
		GetData converter = new GetData();
		for (int i = 0; i < Locations.length; i++)
		{
			Locations[i] = converter.getData(Locations[i]);
		}
		...
		for (int i = 0; i < Locations.length; i++)
		{
			coord1.add(new Coordinate(Locations[i]));
		}
		
		// Create The KML File
		File newFile = new File (FileName);
		
		// Cpoy the data above into the file
		kml.marshal(newFile);
